...
BEGIN
    INSERT INTO users_log_before
        (user_id, login, gravatar_id, url, avatar_url, state)
    VALUES
...
BEGIN
    INSERT INTO users_log_before
        (user_id, login, gravatar_id, url, avatar_url, state)
    VALUES
        (OLD.user_id, OLD.login, OLD.gravatar_id, OLD.url, OLD.avatar_url, 'UPDATE');
END//
...
BEGIN
    INSERT INTO users_log_before
        (user_id, login, gravatar_id, url, avatar_url, state)
    VALUES
        (OLD.user_id, OLD.login, OLD.gravatar_id, OLD.url, OLD.avatar_url, 'DELETE');
END//
...
BEGIN
    INSERT INTO users_log_after
        (user_id, login, gravatar_id, url, avatar_url, state)
    VALUES
        (NEW.user_id, NEW.login, NEW.gravatar_id, NEW.url, NEW.avatar_url, 'UPDATE');
END//
